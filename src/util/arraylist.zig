...
pub fn MultiArrayList(comptime T: type) type {
    return comptime switch (@typeInfo(T)) {
        ...
        .@"union" => UnionMultiArrayList(T),
        else => @compileError("MultiArrayList is only available to structs and tagged unions"),
    };
}

/// Assumes tag names are the same as field names
fn UnionMultiArrayList(comptime T: type) type {
    const info = @typeInfo(T).@"union";
    const fields = info.fields;

    const TagType =
        if (info.tag_type) |tag| tag
        else @compileError("MultiArrayList is only available to tagged unions");
    const Field = std.meta.FieldEnum(T);

    comptime var innerFields: [fields.len + 1]std.builtin.Type.StructField = undefined;
    ...
    return struct {
        ...
        pub const Slice = struct {
            ...
            pub fn items(self: *const Slice, comptime field: std.meta.FieldEnum(Inner)) []@FieldType(T, @tagName(field)) {
                return @field(self.inner, @tagName(field))[0..self.len];
            }

            pub fn get(self: *const Slice, comptime field: Field, index: u32) @FieldType(T, @tagName(field)) {
                assert(index < self.len);
                const tagName = @tagName(field);
                return @field(self.inner, tagName)[index];
            }

            pub fn tag(self: *const Slice) []const TagType {
                return self.inner.tag;
            }

            pub fn capacity(self: *const Slice) u32 {
                return @intCast(@field(self.inner, innerFields[0].name).len);
            }
            ...
            pub fn eql(self: *const Slice, other: *const Slice) bool {
                ...
                for (0..self.len) |i| {
                    inline for (innerFields) |field| {
                        if (@field(self.inner, field.name)[i] != @field(other.inner, field.name)[i]) {
                            return false;
                        }
                    }
                }
        ...
        pub fn init(allocator: Allocator, cap: usize) CompilerError!Self {
            ...
            inline for (innerFields) |field| {
                @field(self.inner, field.name) = allocator.alloc(@typeInfo(field.type).pointer.child, cap) catch return error.AllocatorFailure;
            }
            ...
            inline for (innerFields) |field| {
                ...
                if (@field(self.inner, field.name).len != 0) {
                    if (allocator.remap(@field(self.inner, field.name), cap)) |mem| {
                        ...
                        const mem = allocator.alloc(@typeInfo(field.type).pointer.child, cap) catch return error.AllocatorFailure;
                    ...
                    new = allocator.alloc(@typeInfo(field.type).pointer.child, cap) catch return error.AllocatorFailure;
                }
        ...
        pub fn append(self: *Self, allocator: Allocator, comptime field: Field, element: @FieldType(T, @tagName(field))) CompilerError!void {
            const cap = self.capacity();
        ...
        pub fn items(self: *const Self, comptime field: Field) []@FieldType(T, @tagName(field)) {
            return @field(self.inner, @tagName(field))[0..self.len];
        }

        pub fn tag(self: *const Self) []const TagType {
            return self.inner.tag;
        }

        pub fn get(self: *const Self, comptime field: Field, index: u32) @FieldType(T, @tagName(field)) {
            assert(index < self.len);
            const tagName = @tagName(field);
            return @field(self.inner, tagName)[index];
        }
